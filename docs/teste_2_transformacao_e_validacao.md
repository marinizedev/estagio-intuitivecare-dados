...
A partir do CSV consolidado gerado na etapa 1.3, foi implementada uma etapa de validação dos dados com o objetivo de garantir consistência mínima antes das etapas de enriquecimento e agregação.
...
Para CNPJs inválidos, optou-se por **não descartar os registros**, mas **marcá-los como inválidos por meio de flags de validação**.
...
Este documento descreve, de forma clara e estruturada, todo o raciocínio, decisões técnicas e desafios enfrentados durante a execução dos itens **2.2 (Enriquecimento dos Dados)** e **2.3 (Agregação dos Dados)** do teste técnico.

O objetivo principal foi demonstrar não apenas o resultado final, mas também a **capacidade de análise crítica diante de inconsistências entre o enunciado e os dados reais**, algo comum em cenários reais de mercado.
...
- O arquivo agregado foi gerado corretamente, mas ao abrir o CSV aparentava conter apenas os cabeçalhos.
...
Mais do que gerar arquivos CSV, o processo demonstrou **capacidade analítica, resiliência e adaptação**, competências essenciais para atuação profissional em dados.
